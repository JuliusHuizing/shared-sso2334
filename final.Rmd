...
```{r}
qnorm(0.95, mean_point_estimate, sd(birth_data$birthweight)) # like this? # What does this mean visually on the distribution?

```

(c) Assuming a theoretical mean of 2800 (the null hypothesis) and an alternative hypothesis stating that m \> 2800, one sample, one-tailed t-test yields a p-value of \< 2.2e-16. This means that under the null-hypothesis, it is very unlikely that we would observe the sample data. Assuming a standard confidence interval of 95%, we would reject the null-hypothesis based on this t-test and agree with the expert that the mean birthweight is likely higher than 2800 grams.
